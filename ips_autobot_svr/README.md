...
| `DB_USER` | 数据库用户 | `root` |
| `DB_PASSWORD` | 数据库密码 | `gR7iKWTvQLLqhhwn` |
...
- **表**：`admins`、`users`、`recharge_records`、`admin_login_logs`、`user_login_logs`。DDL 见 `src/db/schema.sql`。
- **初始化**：首次部署执行 `npm run db:init`。若库表已存在且需为用户表增加「用户名」字段，执行一次 `npm run db:migrate-user-username`。
